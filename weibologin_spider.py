...
&su="+su+"&rsakt=mod&checkpin=1&client=ssologin.js(v1.4.18)"
		...
		try:
			...
			print 'Login error!'
		result = urllib2.urlopen('http://weibo.com/Laruence')
		doc = result.read().replace(r'\"',"'").replace(r'\n','').replace(r'\r','').replace(r'\t','').replace(r'<script>','').replace(r'\/','/')
		d = lxml.html.fromstring(doc)
		nodes = d.xpath('//div[@class="WB_detail"]')
		for node in nodes:
			item = WeibologinItem()
			item['content1'] = ' '.join(node.xpath('.//div[@node-type="feed_list_content"]/text() | .//div[@node-type="feed_list_content"]/a/text()'))# extract()是把list转为string，这里是string，所以不需要extract()
			item['content2'] = ' '.join(node.xpath('.//div[@class="WB_info"]/a/text() | .//div[@node-type="feed_list_reason"]/em/text() | .//div[@node-type="feed_list_reason"]/em/a/text()'))
 			item['zhuanfa1'] = ' '.join(node.xpath('.//div[@class="WB_handle"]/a[contains(@action-type,"fl_forward")]/text()'))[3:-1]#.re('\d+'))string没有正则表达式取值，用index取值
			item['zhuanfa2'] = ' '.join(node.xpath('.//div[@mid]/a[2]/text()'))[3:-1]#.re('\d+'))
			item['pinglun1'] = ' '.join(node.xpath('.//div[@class="WB_handle"][1]/a[4]/text()'))[3:-1]#.re('\d+'))
			item['pinglun2'] = ' '.join(node.xpath('.//div[@mid]/a[3]/text()'))[3:-1]#.re('\d+'))
			yield item
