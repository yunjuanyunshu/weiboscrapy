...
#		req = urllib2.Request(url='http://weibo.com/Laruence')
		...
		fff.write(result.read().replace(r'\"',"'").replace(r'\n','').replace(r'\r','').replace(r'\t','').replace(r'<script>FM.view({"ns":"pl.content.homeFeed.index","domid":"Pl_Core_OwnerFeed__3","css":["style/css/module/frameset/comb_PRF_feed.css?version=ad9a30a762a6579d"],"js":"page/js/pl/content/homeFeed/index.js?version=a7747f9c67f85b38","html":','').replace('"','').replace(r'\/','/'))
